...
const Modal: React.FC<ModalProps> = ({ optionIndex, onClose }) => {
  ...
  return (
    <div className="absolute inset-0 bg-black/60 z-20 flex items-end justify-end">
      <div
        className="w-full max-w-6xl h-[450px] rounded-t-xl overflow-hidden relative"
        style={{ backgroundColor: option.backgroundColor }} // Use background color from option
      >
        <button onClick={onClose} className="absolute top-2 right-2 text-white hover:text-white/90">
